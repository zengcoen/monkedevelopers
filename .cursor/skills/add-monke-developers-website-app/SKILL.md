---
name: add-monke-developers-website-app
description: >-
  Add a new app landing page, privacy policy, TOS, and home tile on the
  Monke Developers website (monkedevelopers.com). Use when updating the
  Monke Developers site for a new app, not when creating the iOS Xcode app.
---

# Add a Monke Developers Website App

This skill is **only** for the Monke Developers **website** (`monkedevelopers` repo, GitHub Pages). It does not create or modify the iOS Xcode project.

New site apps are copy-paste of an existing app folder. Do not invent a new page structure.

## Template to copy

Use **`spellingBeeSolver/`** as the template (landing page, screenshots, App Store badge, privacy policy, terms). If that folder is missing, use `wordHuntSolver/`.
...
1. **Copy the folder**
   - Duplicate `spellingBeeSolver/` → `yourAppName/` (camelCase folder name, matching existing apps).
...
2. **Generate descriptions from the app name**
   ...
   - Write **landing-page copy**: a short intro paragraph, 2–3 feature bullets, and a closing paragraph — same shape as `spellingBeeSolver/index.html`.
   - Do not leave Spelling Bee Solver placeholder text. Do not ask the user for marketing copy unless they already provided it.
...
3. **Update the copied files**
   ...
   - `yourAppName/privacyPolicy/index.html`: replace every "Spelling Bee Solver" mention with the new app name.
...
- [ ] Folder copied from `spellingBeeSolver/`
